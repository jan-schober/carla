...
def filter_angle_distance(vehicles_list, sensor, max_dist=100):
    ...
    vehicles_list, vehicles_transform, vehicles_transform_s = filter_distance(vehicles_list, vehicles_transform,
                                                                              vehicles_transform_s, sensor, max_dist)
    vehicles_list, vehicles_transform, vehicles_transform_s = filter_angle(vehicles_list, vehicles_transform,
                                                                           vehicles_transform_s, sensor)
...
def filter_lidar(lidar_data, camera, max_dist):
    ...
    selector = np.array(
        np.logical_and(dist2 < (max_dist ** 2), np.logical_and(p_angle_h < (CAM_HFOV / 2), p_angle_v < (CAM_VFOV / 2))))
